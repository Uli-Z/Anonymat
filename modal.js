...
(function (window) {
  ...
  function openCustomPlaceholderModal(selectedText) {
    ...
    if (modal) {
      // Show modal first so focus works
      modal.style.display = "block";
  ...
  function updatePlaceholderOptions() {
    ...
    if (inactivePlaceholders.length > 0) {
      ...
      const headerInactive = document.createElement("div");
      const titleInactive = document.createElement("h4");
      titleInactive.textContent = window.translate("contextMenu.additionalPlaceholders") || "Weitere Platzhalter";
      const toggleButton = document.createElement("button");
      toggleButton.textContent = window.translate("contextMenu.showAdditionalPlaceholders") || "Anzeigen";
      toggleButton.classList.add("toggle-button");
      headerInactive.appendChild(titleInactive);
  ...
  document.addEventListener("DOMContentLoaded", function() {
    if (!localStorage.getItem("firstRunAcknowledged")) {
      ...
      // reuse language selection from general modal
      const generalSelect = document.getElementById("languageSelect");
      const generalLabel = document.querySelector("label[for='languageSelect']");
      if (generalSelect && generalLabel && languageContainer) {
        const clonedLabel = generalLabel.cloneNode(true);
        const clonedSelect = generalSelect.cloneNode(true);
  ...
  // Make any modal dismissable by clicking outside its content
